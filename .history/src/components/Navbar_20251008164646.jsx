...
const Navbar = () => {
  ...
  return (
    <div className="w-full neue">
      {/* Main Navbar - Remove mix-blend-difference when menu is open */}
      <nav className={`fixed backdrop-blur-md transition-all duration-500 top-2 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] md:w-[calc(100%-4rem)] max-w-[112rem] rounded-lg px-8 z-[999999] ${
        menuOpen ? '' : 'mix-blend-difference'
      }`}>
        {/* Header */}
        <div className="flex items-center justify-between py-5 relative z-[60]">
          {/* Left - Logo */}
          <div className={`text-2xl z-[99999999999999999999999999999999] font-[500] tracking-wide text-white transition-opacity duration-300 ${menuOpen ? 'opacity-0' : 'opacity-100'}`}>
            mangofx
          </div>
