...
const Navbar = () => {
  ...
  return (
    <div className="w-full neue">
      ...
      {(menuOpen || isClosing) && (
        <div className={`fixed inset-0 z-40 flex flex-col ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          {/* Background without blurring the top bar */}
          <div className={`absolute inset-0 bg-black/80 backdrop-blur-3xl ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
            <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-transparent via-black/50 to-black/80 ${isClosing ? 'animate-radialFadeOut' : 'animate-radialFadeIn'}`}></div>
          </div>
          
          {/* Content Container */}
          <div className={`relative z-10 flex flex-col h-full ${isClosing ? 'animate-slideDown' : 'animate-slideUp'}`}>
            {/* Top Bar - Separate from blurred background */}
      ...
      <style jsx>{`
        ...
        .animate-radialFadeIn {
          animation: radialFadeIn 0.6s ease-out forwards;
        }
        .animate-radialFadeOut {
          animation: radialFadeOut 0.6s ease-out forwards;
        }
        .animate-slideUp {
